...
//	}
	...
	protected void update(double ds) {
		...
		if (AD.men(null).get(this) == 0) {
			if (region() == null || region().faction() != faction()) {
				remove();
				return;
			}else if (state() != WArmyState.fortifying) {
				if (state() != WArmyState.fortified)
		...
		if (faction() == FACTIONS.player() && region() != null && region().faction() instanceof FactionNPC) {
			...
			if (!DIP.get(ff).ally)
				ROPINION.STANCE().tresPass(ff, ds);
		}
	...
	private void returnResources() {
		Shipment ship = WORLD.ENTITIES().caravans.create(ctx(), cty(), FACTIONS.player().capitolRegion(), TRADE_TYPE.spoils);
		if (ship != null) {
			for (ADSupply ss : AD.supplies().all) {
				int am = (int) (ss.current().get(this)-ss.targetAmount(this));
				am = CLAMP.i(am, 0, Short.MAX_VALUE);
				if (am > 0) {
					ship.loadAndReserve(TR.get(ss.res), am);
//					if (ss.baseHealth == 0)
//						ship.loadAndReserve(ss.res, am);
	...
	public void setDestination(int tx, int ty) {
		...
		if (path.find(ctx(), cty(), tx, ty)) {
			...
			CoopRuntime.armyMove(this, tx, ty);
		}
	}
	...
	public void besiege(Region reg) {
		if (AD.men(null).get(this) == 0)
			return;
		CoopRuntime.armyBesiege(this, reg);
		if (besieging(reg)) {
	...
	public void raid(Region reg) {
		if (AD.men(null).get(this) == 0)
			return;
		CoopRuntime.armyRaidRegion(this, reg);
	...
	public void raid(boolean raid) {
		...
		if (raid == raiding())
			return;
		CoopRuntime.armyRaidToggle(this, raid);
	...
	public void intercept(WArmy other) {
		...
		if (path.find(ctx(), cty(), other.ctx(), other.cty())) {
			...
			CoopRuntime.armyIntercept(this, other);
		}
	}
