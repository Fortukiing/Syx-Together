...
final class ModuleEmployment implements ModuleMaker {
	...
	private final CharSequence ¤¤mark = "¤Click to apply for {0} subjects";
	private final CharSequence ¤¤unmark = "¤Click to cancel for {0} subjects. ({1} subjects are currently active).";
	...
	private class I extends UIRoomModule {
		...
		public void appendManageScr(GGrid grid, GGrid text, GuiSection sExta) {
			...
			grid.add(new GStat() {
				...
				@Override
				public void update(GText text) {
					double emp = blueprint.employment().employed()-150;
					if (emp <= 0)
						return;
					emp = Math.pow(emp, 1.2);
					GFORMAT.perc(text, emp*blueprint.employment().accidentsPerYear / (BOOSTABLES.CIVICS().ACCIDENT.get(HCLASS_RACE.clP())), 2);
					text.normalify();
			...
			if (blueprint instanceof ROOM_EMPLOY_AUTO) {
				CLICKABLE c = new GButt.Checkbox() {
					...
					protected void clickA() {
						...
						CoopRuntime.roomAutoEmployChanged(get.get(), !b);
					}
					@Override
			...
			if (blueprint.employmentExtra() != null) {
				...
				r = new GStat() {
					...
					public void hoverInfoGet(GBox b) {
						...
						b.NL();
						if (S.get().developer) {
							b.add(b.text().add(g(get).employees().fetchSecondsPerPerson()));
						}
	...
	private CLICKABLE cm (StatMultiplierAction mm, GETTER<RoomInstance> get) {
		...
		CLICKABLE c = new GButt.ButtPanel(mm.icon) {
			...
			protected void renAction() {
				...
				for (Humanoid h : get.get().employees().employees()) {
					if (mm.canBeMarked(h.indu()))
						...
						active++;
						if (mm.consumeIs(h.indu()))
							consuming++;
					}
				}
				
				activeSet(markable > 0);
				selectedSet(active > 0);
			};
			...
			public void hoverInfoGet(GUI_BOX text) {
				...
				if (active > 0) {
					b.text(b.text().add(¤¤unmark).insert(0, active).insert(1, consuming));
				}else {
					b.text(b.text().add(¤¤mark).insert(0, markable));
				}
